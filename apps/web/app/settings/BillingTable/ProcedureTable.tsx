...
export default function ProcedureTable({
  ...
  const [data, setData] = useState<ProcedureItem[]>([]);
  ...
  // ✅ Fetch procedure data with pagination
  const fetchProcedures = async (pageNumber = 1) => {
    try {
      setLoading(true);
      const res = await GetBillingItem({
        chargeType: "procedure", // ✅ Correct filter
        page: pageNumber,
        limit,
      });
  ...
  return (
    <>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-100">
              <TableHead className="w-[40px] text-center">S.No</TableHead>
          ...
          <TableBody>
            {loading ? (
              ...
              data.map((item: ProcedureItem, idx: number) => (
                <TableRow
      ...
      {/* 🗑️ Animated Delete Confirmation Dialog */}
              ...
              <AlertDialogAction
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    fetchProcedures(page); // refresh list after delete
                  } catch (error: any) {
                    toast.current?.show({
                      severity: "error",
