...
interface OtherCharge {
  ...
  chargeType?: { BillItemTypeName?: string | null };
  isActive?: boolean;
}
...
export default function OtherChargesTable({
  onEdit,
  refreshTrigger,
}: OtherChargesTableProps) {
  const [data, setData] = useState<OtherCharge[]>([]);
  const [loading, setLoading] = useState(false);
  const toast = useRef<Toast>(null);

  const [deleteTarget, setDeleteTarget] = useState<OtherCharge | null>(null);
  ...
  // ✅ Fetch Other Charges (Paginated)
  const fetchOtherCharges = async (pageNumber = 1) => {
    try {
      setLoading(true);
      const res = await GetBillingItem({
        chargeType: "other charges", // ✅ correct filter
        page: pageNumber,
        limit,
      });
            ...
            <AlertDialogFooter className="flex justify-end gap-2 mt-6">
              ...
              <AlertDialogAction
                ...
                onClick={async () => {
                  ...
                  try {
                    await deleteBillingItemCharge(deleteTarget.BillingItemChargeId);
                    toast.current?.show({
                      severity: "success",
                      summary: "Deleted Successfully",
                      life: 2000,
                    });
                    fetchOtherCharges(page); // ✅ Refresh current page
                  } catch (error: any) {
                    toast.current?.show({
                      severity: "error",
