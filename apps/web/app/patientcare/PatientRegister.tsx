...
import { ChevronsUpDown, Check, ChevronRight, ChevronsRight ,Loader2Icon } from "lucide-react";
...
export function RegisterPatient() {
  ...
  const isPartialValid = Boolean(
    watchedFields[0]?.trim() && // Title
      ...
      watchedFields[7]?.trim() // doorNumber
  );
  //KIN and Emergency
  const relations = [
    "Father",
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const [allergyData, languageData, historyData] = await Promise.all([
          fetchPatientAllergies(),
          fetchLanguagesSpoken(),
          fetchPastMedical(),
        ]);
        ...
        console.log("Medical History API Response:", historyData);
        ...
        setPastMedicalHistory(
          historyData?.return?.map((h: any) => h.name) || []
        );
      } catch (error) {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          dispatch(fetchAllRegisterPatient({ page: 1, limit: 10 }));
          reset();
          setImageUrl(null);
        }, 5000);
      }, 800);
