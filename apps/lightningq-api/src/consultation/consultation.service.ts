...
export class ConsultationService {
  ...
  async addOrUpdateConsultation(
    ...
    // ✅ Perform only database operations in transaction
    const consultationResult = await this.prisma.$transaction(
      async (tx) => {
        let result;
        ...
        if (appointment?.patient?.email) {
          ...
          if (!pdfBuffer) {
            ...
            await this.mailerService.sendMail(
              ...
              `<p>Dear ${patientName},</p><p>Your consultation summary could not be attached.</p>`,
            );
          } else {
